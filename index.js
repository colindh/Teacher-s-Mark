...
// Get Courses list
app.get('/courses', requireToken(), function(req, res) {
  SLC_app.api("/courseOfferings", "GET", req.session.tokenId, {}, {}, function (data) {
    res.json(data);
  })
});

// Get Courses list
app.get('/sections', requireToken(), function(req, res) {
  SLC_app.api("/sections", "GET", req.session.tokenId, {}, {}, function (data) {
    res.json(data);
  })
});
...
app.post('/attendances', requireToken(), function(req, res) {
    var d = new Date();
  SLC_app.api("attendances?studentId="+req.param('student_id'),"GET",req.session.tokenId,{},{},function(data){
    console.log(JSON.stringify(data));
    data[0]["schoolYearAttendance"].push({
      "schoolYear": "2012-2013",
          "attendanceEvent":[{
            "date": '2013-06-02',
	    "event": 'In Attendance',
           "reason":req.param('message')
          }]});
    console.log(JSON.stringify(data));
    SLC_app.api("/attendances/"+data[0]["id"], "PUT", req.session.tokenId, {}, 
    data[0], function (newData) {
      console.log(newData);
      res.json(newData);
    });
  });
